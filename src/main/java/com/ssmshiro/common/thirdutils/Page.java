package com.ssmshiro.common.thirdutils;
...
public class Page<T> implements Serializable{
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		// 不同的请求创建不同的cookie来记录分页信息
		String requestUri = request.getRequestURI();
		String viewName = "";
		if(StringUtils.isNotEmpty(requestUri)){
			viewName = StringUtils.replace(requestUri, "/", "");
		}
		...
		if (StringUtils.isNumeric(no)){
			...
			no = CookieUtils.getCookie(request, viewName + "pageNo");
			if (StringUtils.isNumeric(no)){
				this.setPageNo(Integer.parseInt(no));
			}
		}
		...
		if(defaultPageSize == -1){
			...
			no = CookieUtils.getCookie(request, viewName + "pageSize");
		...
		if(StringUtils.isNumeric(String.valueOf(this.pageNo))){
			CookieUtils.setCookie(response, viewName + "pageNo", String.valueOf(this.pageNo));
		}
		if(StringUtils.isNumeric(String.valueOf(this.pageSize))){
			CookieUtils.setCookie(response, viewName + "pageSize", String.valueOf(this.pageSize));
		}
		// 设置排序参数
